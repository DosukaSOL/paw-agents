...
def is_dog_pixel(rgba: tuple[int, int, int, int]) -> bool:
    # The source sheet already encodes background as alpha=0 and the dog body
    # as alpha=255 (no partial alpha). Trust that — any earlier per-pixel
    # white-stripping was destroying the dog's white fur, eye highlights and
    # paw socks because those are also near-white.
    return rgba[3] >= 16
...
def main() -> None:
    ...
    # The source sheet already has alpha=0 for background and alpha=255 for
    # the dog body — no background processing needed. Earlier versions of
    # this script tried to strip near-white pixels which destroyed the
    # dog's white belly fur, paw socks and eye highlights.
    spx = sheet.load()
