// ─── Purp SCL Language Integration (PAW v4.0, upstream compat: v2.2.0 PINOCCHIO) ───
// Supports the full Purp Smart Contract Language syntax (compatible with purp-scl v2.2.0).
...
// v2.1.0: account resolution, Clock sysvar, PDA validation, frontend hooks, e2e tests.
// v2.2.0: state machine DSL, parser error recovery, expanded linter rules, LSP enhancements, publish command.
...
export class PurpEngine {
  // ─── Purp v2.2.0 metadata ───
  static readonly UPSTREAM_VERSION = '2.2.0';
  ...
  // ─── Parse native Purp v2.2.0 syntax ───
  ...
  // ─── Compile Purp v2.2.0 to Pinocchio Rust + TypeScript + Frontend ───
  ...
  // ─── Generate Pinocchio Rust from Purp (v2.2.0) ───
  private generatePinocchioRust(program: PurpProgram): string {
    ...
    const lines: string[] = [
      `// Auto-generated by Purp SCL v2.2.0 Compiler (Pinocchio)`,
      `// Source: ${program.name}`,
      ``,
      `use pinocchio::{`,
  ...
  // ─── Generate TypeScript SDK from Purp (Pinocchio v2.2.0) ───
  private generateTypeScriptSDK(program: PurpProgram): string {
    const lines: string[] = [
      `// Auto-generated TypeScript SDK by Purp SCL v2.2.0 Compiler (Pinocchio)`,
      `// Source: ${program.name}`,
      ``,
      `import {`,
  ...
  // ─── Solana IDL JSON (v2.2.0 / Pinocchio-compatible) ───
  ...
  private transpileBodyLine(line: string): string {
    // Purp v2.2.0 → Pinocchio Rust transpilation
    return line
  ...
  private generateFrontendOutput(program: PurpProgram): string {
    const lines: string[] = [
      `// Auto-generated Frontend by Purp SCL v2.2.0 Compiler (Pinocchio)`,
      `// Source: ${program.name}`,
      ``,
