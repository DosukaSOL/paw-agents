// ─── Purp SCL v2.2.0 Compatibility Tests ───
...
describe('Purp v1.2.1 — Metadata & Constants', () => {
  test('upstream version is 2.2.0', () => {
    expect(PurpEngine.UPSTREAM_VERSION).toBe('2.2.0');
  });
...
`;
    ...
    expect(result.frontend_output).toContain('Purp SCL v2.2.0');
...
`;
  ...
  test('Rust output header references v2.2.0', () => {
    const source = `
program TestProg {
}
...
`;
    const program = purp.parse(source) as any;
    const result = purp.compile(program);
    expect(result.rust_output).toContain('v2.2.0');
  });

  test('TypeScript SDK header references v2.2.0', () => {
    const source = `
program TestProg {
}
...
`;
    const program = purp.parse(source) as any;
    const result = purp.compile(program);
    expect(result.typescript_sdk).toContain('v2.2.0');
  });
